...
- Evaluation: pymatgen (`eval.csv` includes `parse_ok`, `reduced_formula`, `spacegroup_number/symbol`, `error`)  
...
`python scripts/demo6_summarize_from_eval.py \   --root reports/demo6_prompt_sweep_n100 \   --out_csv reports/demo6_prompt_sweep_n100/summary.csv \   --out_md  reports/demo6_prompt_sweep_n100/summary.md \   --topk 8`
...
`IN_DIR=data/official/tokens_v1_train_val/mp_oqmd_nomad_cifs_semisymm_Z_props_2 OUT_DIR=data/datasets/mix10000_tokenized  python scripts/subset_tokenized_by_data_prefix.py \   --in_dir "$IN_DIR" \   --out_dir "$OUT_DIR" \   --n_train 9000 \   --n_val 1000 \   --min_len 32`
...
`mkdir -p out/mix10000_ft_small_v2 cp -f external/CrystaLLM/crystallm_v1_small/ckpt.pt out/mix10000_ft_small_v2/ckpt.pt`
